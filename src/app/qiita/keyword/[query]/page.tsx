...
export default function QiitaKeywordPage() {
  ...
  const { data, isLoading, error } = useFetch<{ articles: QiitaArticle[] }>(
    query ? `/api/qiita/keyword?q=${encodeURIComponent(query)}` : null,
    'Error fetching Qiita keyword results'
  );
  ...
  useEffect(() => {
    if (query) {
      document.title = `「${query}」の検索結果 | Qiita | PrepFeed`;
    }
  }, [query]);
  ...
  const handleKeywordSearch = (e: React.FormEvent) => {
    ...
    if (keywordQuery.trim()) {
      router.push(
        `/qiita/keyword/${encodeURIComponent(keywordQuery.trim())}`
      );
    }
  };
        ...
        <form onSubmit={handleKeywordSearch} className="w-full">
          <div className="relative">
            <input
              ...
              className="w-full pl-12 pr-24 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-[#55C500] focus:border-transparent transition-all duration-200"
            />
