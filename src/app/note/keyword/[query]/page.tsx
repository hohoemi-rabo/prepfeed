    ...
    const fetchResults = async () => {
      ...
      try {
        const response = await fetch(
          `/api/note/keyword?q=${encodeURIComponent(query)}`
        );
        ...
        console.error('Error fetching note keyword results:', err);
        const errorMessage =
      ...
      <div className="mb-6">
        <form onSubmit={handleKeywordSearch} className="w-full">
          <div className="relative">
            ...
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-gradient-to-r from-[#41C9B4] to-[#2FA898] text-white rounded-md text-sm font-medium hover:opacity-90 transition-opacity"
            >
              検索
            </button>
          </div>
        ...
        <div className="card">
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0">
              <div className="w-12 h-12 bg-gradient-to-r from-[#41C9B4] to-[#2FA898] rounded-lg flex items-center justify-center">
